#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════
# Obelysk Pipeline — Step 8: Privacy Withdraw (Unshield Tokens)
# ═══════════════════════════════════════════════════════════════════════
#
# Generates a withdrawal proof to unshield tokens from the privacy pool
# and optionally submits it on-chain.
#
# Usage:
#   bash scripts/pipeline/08_privacy_withdraw.sh --amount 1000 --dry-run
#   bash scripts/pipeline/08_privacy_withdraw.sh --amount 1000 --submit
#
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Unshield tokens from the VM31 privacy pool."
            echo ""
            echo "Required:"
            echo "  --amount NUM       Amount to withdraw"
            echo ""
...
if [[ -z "$OUTPUT_PATH" ]]; then
    OUTPUT_PATH="${OBELYSK_DIR}/privacy/withdraw_proof.json"
...
banner
echo -e "${BOLD}  Privacy Withdraw${NC}"
echo ""
log "Amount:     ${AMOUNT}"
log "Asset:      ${ASSET_ID}"
log "Network:    ${NETWORK}"
...
timer_start "withdraw"

# ─── Build CLI args ──────────────────────────────────────────────────

CLI_ARGS=("withdraw" "--amount" "$AMOUNT" "--asset" "$ASSET_ID" "--output" "$OUTPUT_PATH")
...
header "Generating Withdraw Proof"
...
ok "Withdraw proof saved to ${OUTPUT_PATH}"
...
" 2>/dev/null || echo "")
    ...
    ok "Withdrawal submitted on-chain"
fi

# ─── Summary ─────────────────────────────────────────────────────────

ELAPSED=$(timer_elapsed "withdraw")
echo ""
echo -e "${GREEN}${BOLD}"
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║  PRIVACY WITHDRAW COMPLETE                           ║"
echo "  ╠══════════════════════════════════════════════════════╣"
printf "  ║  Amount:       %-36s ║\n" "${AMOUNT}"
printf "  ║  Asset:        %-36s ║\n" "${ASSET_ID}"
printf "  ║  Network:      %-36s ║\n" "${NETWORK}"
