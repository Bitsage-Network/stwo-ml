#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════
# Obelysk Pipeline — Step 6: Privacy Deposit (Shield Tokens)
# ═══════════════════════════════════════════════════════════════════════
#
# Generates a shielded deposit proof and optionally submits it on-chain.
#
# Usage:
#   bash scripts/pipeline/06_privacy_deposit.sh --amount 1000 --dry-run
#   bash scripts/pipeline/06_privacy_deposit.sh --amount 1000 --submit
#
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --recipient)  RECIPIENT_PUBKEY="$2"; shift 2 ;;
        ...
        -h|--help)
            ...
            echo ""
            echo "Shield tokens into the VM31 privacy pool."
            echo ""
            echo "Required:"
            echo "  --amount NUM       Amount to deposit"
            echo ""
            ...
            echo "  --recipient HEX    Recipient public key hex (default: self)"
...
if [[ -z "$OUTPUT_PATH" ]]; then
    OUTPUT_PATH="${OBELYSK_DIR}/privacy/deposit_proof.json"
...
banner
echo -e "${BOLD}  Privacy Deposit${NC}"
echo ""
log "Amount:     ${AMOUNT}"
log "Asset:      ${ASSET_ID}"
log "Network:    ${NETWORK}"
...
CLI_ARGS=("deposit" "--amount" "$AMOUNT" "--asset" "$ASSET_ID" "--output" "$OUTPUT_PATH")
...
if [[ -n "$RECIPIENT_PUBKEY" ]]; then
    CLI_ARGS+=("--recipient" "$RECIPIENT_PUBKEY")
fi
...
header "Generating Deposit Proof"
...
ok "Deposit proof saved to ${OUTPUT_PATH}"
...
if [[ "$DO_SUBMIT" == "true" ]]; then
    ...
    # Extract calldata from proof JSON
    CALLDATA=$(python3 -c "
import json, sys
...
" 2>/dev/null || echo "")
    ...
    ok "Deposit submitted on-chain"
fi

# ─── Summary ─────────────────────────────────────────────────────────

ELAPSED=$(timer_elapsed "deposit")
echo ""
echo -e "${GREEN}${BOLD}"
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║  PRIVACY DEPOSIT COMPLETE                            ║"
echo "  ╠══════════════════════════════════════════════════════╣"
printf "  ║  Amount:       %-36s ║\n" "${AMOUNT}"
printf "  ║  Asset:        %-36s ║\n" "${ASSET_ID}"
printf "  ║  Network:      %-36s ║\n" "${NETWORK}"
