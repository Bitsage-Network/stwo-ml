...
use crate::components::attention::{AttentionWeights, attention_forward};
    ...
    FrameworkComponent<ActivationEval>: ComponentProver<B>,
{
    ...
    for node in &graph.nodes {
        match &node.op {
            ...
            GraphOp::Attention { config: attn_cfg } => {
                ...
                let inter = attention_forward(&current, &attn_w, attn_cfg, false);
                intermediates.push((node.id, current.clone()));
    ...
    let log_size = activation_layers.iter()
        .map(|l| l.table.log_size)
        .max()
        .unwrap_or(4)
        .max(4); // SIMD minimum
