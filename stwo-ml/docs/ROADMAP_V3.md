...
**Description**: Deploy the `prove-server` binary on the A10G instance behind `api.obelysk.com`. Add health checks, rate limiting, API key authentication, and monitoring.
...
**Dependencies**: A10G instance is already running. DNS for `api.obelysk.com` must be configured.
...
**Acceptance criteria**:
- `curl https://api.obelysk.com/health` returns 200 with server status.
- `curl -X POST https://api.obelysk.com/api/v1/prove -H "X-API-Key: ..." -d '{"model":"smollm2-135m","input":"Hello"}'` returns a valid proof.
