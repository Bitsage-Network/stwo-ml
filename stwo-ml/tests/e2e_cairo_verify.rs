...
fn test_gkr_roundtrip_matmul_only() {
    ...
    match &gkr.layer_proofs[0] {
        ...
        other => panic!("expected MatMul layer proof, got {:?}", std::mem::discriminant(other)),
    }
    ...
    assert_eq!(calldata[0], FieldElement::from(1u64), "num_layers should be 1");
    assert_eq!(calldata[1], FieldElement::from(0u64), "tag should be 0 (MatMul)");
    assert_eq!(calldata[2], FieldElement::from(2u64), "num_rounds should be 2");
    ...
    assert!(calldata.len() > 30, "calldata should have substantial content: {} felts", calldata.len());

    // Verify weight commitments populated (B5.2)
    assert_eq!(
        gkr.weight_commitments.len(), 1,
        "single matmul should have 1 weight commitment"
    );
    assert_ne!(
        gkr.weight_commitments[0], FieldElement::ZERO,
...
fn test_gkr_roundtrip_with_activation() {
    ...
    // Prove
    let proof = prove_model_pure_gkr(&graph, &input, &weights)
        .expect("GKR MLP proving should succeed");
    let gkr = proof.gkr_proof.as_ref().expect("should have GKR proof");
    ...
    let tags: Vec<&str> = gkr.layer_proofs.iter().map(|lp| match lp {
    ...
    assert_eq!(calldata[0], FieldElement::from(3u64), "num_layers should be 3");
    ...
    // Layer 0: MatMul
    assert_eq!(calldata[offset], FieldElement::from(0u64), "layer 0 should be MatMul (tag 0)");
    ...
    // Layer 1: Activation
    assert_eq!(calldata[offset], FieldElement::from(3u64), "layer 1 should be Activation (tag 3)");
...
fn test_gkr_rejects_tampered_round_poly() {
    ...
    match &mut gkr.layer_proofs[0] {
        stwo_ml::gkr::types::LayerProof::MatMul { round_polys, .. } => {
            ...
            round_polys[0].c0 = orig + stwo::core::fields::qm31::QM31::from_u32_unchecked(1, 0, 0, 0);
        }
...
fn test_gkr_rejects_tampered_final_eval() {
    ...
    match &mut gkr.layer_proofs[0] {
        stwo_ml::gkr::types::LayerProof::MatMul { final_a_eval, .. } => {
            *final_a_eval = *final_a_eval + stwo::core::fields::qm31::QM31::from_u32_unchecked(1, 0, 0, 0);
        }
        _ => panic!("expected MatMul"),
    }
...
fn test_gkr_serialization_deterministic() {
    ...
    assert_eq!(cd1.len(), cd2.len(), "serialized length should be deterministic");
...
fn test_gkr_rejects_wrong_input_claim() {
    ...
    gkr.input_claim.value = gkr.input_claim.value
...
fn test_gkr_rejects_wrong_weight_commitment() {
    ...
    let _gkr_sn = build_gkr_starknet_proof(&proof, model_id, &input)
...
fn test_gkr_rejects_tampered_activation_logup() {
    ...
    assert!(found, "should have found an Activation layer with LogUp proof to tamper");
...
fn test_gkr_roundtrip_with_layernorm() {
    ...
    match &gkr.layer_proofs[1] {
        ...
        other => panic!("expected LayerNorm proof, got {:?}", std::mem::discriminant(other)),
    }
    ...
    assert_eq!(calldata[offset], FieldElement::from(0u64), "first layer should be MatMul");
    let nr0: u64 = calldata[offset + 1].try_into().expect("num_rounds");
    offset += 2 + (nr0 as usize) * 12 + 4 + 4;

    // Layer 1: LayerNorm (tag=4)
    assert_eq!(
        calldata[offset], FieldElement::from(4u64),
...
fn test_sp3_export_mlp_proof_calldata() {
    ...
    match &mut tampered_gkr.layer_proofs[0] {
        stwo_ml::gkr::types::LayerProof::MatMul { round_polys, .. } => {
            round_polys[0].c0 = round_polys[0].c0
                + stwo::core::fields::qm31::QM31::from_u32_unchecked(1, 0, 0, 0);
        }
    ...
    let tampered_result = stwo_ml::gkr::verify_gkr(
        &circuit, &tampered_gkr, &proof.execution.output, &mut ch2,
    );
    assert!(tampered_result.is_err(), "tampered proof must fail verification");
    ...
    std::fs::write(&tampered_path, serde_json::to_string_pretty(&tampered_json).unwrap())
        .expect("failed to write tampered artifact");

    eprintln!("SP3.4 tampered artifact written to: {}", tampered_path.display());
}
...
fn test_sp3_export_matmul_only_proof() {
    ...
    let qm31_parts = |v: stwo::core::fields::qm31::SecureField| -> [u64; 4] {
        ...
        [comps[0].0 as u64, comps[1].0 as u64, comps[2].0 as u64, comps[3].0 as u64]
    };

    // Capture weight claims for cross-language verification
    let weight_claims_json: Vec<_> = gkr.weight_claims.iter().map(|c| {
        serde_json::json!({
...
fn test_verify_model_gkr_calldata_matmul_only() {
    use stwo_ml::starknet::{
        build_verify_model_gkr_calldata, extract_matmul_dims, extract_dequantize_bits,
        build_circuit_descriptor, build_register_gkr_calldata,
    };
    ...
    let reg_calldata = build_register_gkr_calldata(
        model_id,
        &gkr.weight_commitments,
        &desc,
    );
    ...
    let verify_calldata = build_verify_model_gkr_calldata(
        gkr, &circuit, model_id, &raw_io_data,
    );
    assert!(verify_calldata.total_felts > 10, "should have substantial calldata");
...
fn test_verify_model_gkr_calldata_mlp_relu() {
    ...
    assert!(verify_calldata.total_felts > 50, "MLP calldata should be larger");
...
fn test_d7_export_onchain_calldata() {
    ...
    std::fs::write(&register_path, register_calldata.join(" "))
        .expect("write register calldata");
    ...
    eprintln!("  Register: {} ({} parts)", register_path.display(), register_calldata.len());
    eprintln!("  Verify:   {} ({} parts)", verify_path.display(), verify_calldata.total_felts);
...
fn test_d9_export_mlp_onchain_calldata() {
    use stwo_ml::starknet::{
        build_verify_model_gkr_calldata, build_register_gkr_calldata,
        build_circuit_descriptor, extract_matmul_dims, extract_dequantize_bits,
    };
...
fn test_d10_export_layernorm_onchain_calldata() {
    ...
    let proof = prove_model_pure_gkr(&graph, &input, &weights)
        .expect("LayerNorm proving should succeed");
    let gkr = proof.gkr_proof.as_ref().unwrap();
    let circuit = stwo_ml::gkr::LayeredCircuit::from_graph(&graph).unwrap();
    ...
    // Build calldata
    let register_calldata = build_register_gkr_calldata(
        model_id, &gkr.weight_commitments, &circuit_desc,
    );
    let raw_io_data = stwo_ml::cairo_serde::serialize_raw_io(&input, &proof.execution.output);
    let verify_calldata = build_verify_model_gkr_calldata(
        gkr, &circuit, model_id, &raw_io_data,
...
fn test_d11_export_residual_onchain_calldata() {
    ...
    let mut weights = stwo_ml::compiler::graph::GraphWeights::new();
    let mut w0 = M31Matrix::new(4, 4);
    for i in 0..4 { w0.set(i, i, M31::from(1u32)); } // identity
    weights.add_weight(0, w0);
    let mut w2 = M31Matrix::new(4, 4);
    for i in 0..4 { for j in 0..4 { w2.set(i, j, M31::from(((i + j) % 3 + 1) as u32)); } }
    weights.add_weight(2, w2);
    ...
    let proof = prove_model_pure_gkr(&graph, &input, &weights)
        .expect("Residual proving should succeed");
    let gkr = proof.gkr_proof.as_ref().unwrap();
    let circuit = stwo_ml::gkr::LayeredCircuit::from_graph(&graph).unwrap();
    ...
    // Verify the GKR proof with Rust verifier
    {
        ...
        let result = stwo_ml::gkr::verify_gkr(
            &circuit, gkr, &proof.execution.output, &mut verify_ch,
        );
