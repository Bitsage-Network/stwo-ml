...
/// GPU-accelerated LayerNorm with CPU fallback.
///
...
pub(crate) fn apply_layernorm_detailed_auto(
    ...
    #[cfg(feature = "cuda-runtime")]
    {
        ...
        let inv_n = m31_mod_inverse(n as u32);
        let rsqrt_table = build_rsqrt_table(LayerNormConfig::new(dim).rsqrt_table_log_size);
        let gpu_table = rsqrt_table.as_gpu_lookup_table();
        match crate::gpu_sumcheck::gpu_layernorm_m31(input, dim, inv_n.0, &gpu_table) {
            Ok((output_matrix, means, variances, rsqrt_vals)) => {
                let cols = input.cols;
                let total = input.rows * cols;
                let mut all_means = Vec::with_capacity(total);
                let mut all_vars = Vec::with_capacity(total);
                let mut all_rsqrt = Vec::with_capacity(total);
                for row in 0..input.rows {
                    for col in 0..cols {
                        if col < n {
                            all_means.push(means[row]);
                            all_vars.push(variances[row]);
                            all_rsqrt.push(rsqrt_vals[row]);
                        } else {
                            all_means.push(M31::from(0));
                            all_vars.push(M31::from(0));
                            all_rsqrt.push(M31::from(1u32 << 16));
                        }
                    }
                }
                return LayerNormIntermediates {
                    inputs: input.data.clone(),
                    means: all_means,
                    variances: all_vars,
                    rsqrt_vals: all_rsqrt,
                    outputs: output_matrix.data.clone(),
                    output_matrix,
                };
            }
            Err(e) => eprintln!("[layernorm] GPU failed, CPU fallback: {e}"),
        }
    }
    apply_layernorm_detailed(input, dim)
}

/// GPU-accelerated RMSNorm with CPU fallback.
///
/// Tries the GPU path first (cuda-runtime feature). On failure or when CUDA
/// is unavailable, falls back to the CPU `apply_rmsnorm_detailed`.
pub(crate) fn apply_rmsnorm_detailed_auto(input: &M31Matrix, dim: usize) -> RMSNormIntermediates {
    #[cfg(feature = "cuda-runtime")]
    {
        ...
        let n = dim.min(input.cols);
        let inv_n = m31_mod_inverse(n as u32);
        let rsqrt_table = build_rmsnorm_rsqrt_table(RMSNormConfig::new(dim).rsqrt_table_log_size);
        let gpu_table = rsqrt_table.as_gpu_lookup_table();
        match crate::gpu_sumcheck::gpu_rmsnorm_m31(input, dim, inv_n.0, &gpu_table) {
            Ok((output_matrix, rms_sq_vals, rsqrt_vals)) => {
                let cols = input.cols;
                let total = input.rows * cols;
                let mut all_rms = Vec::with_capacity(total);
                let mut all_rsqrt = Vec::with_capacity(total);
                for row in 0..input.rows {
                    for col in 0..cols {
                        if col < n {
                            all_rms.push(rms_sq_vals[row]);
                            all_rsqrt.push(rsqrt_vals[row]);
                        } else {
                            all_rms.push(M31::from(0));
                            all_rsqrt.push(M31::from(1u32 << 16));
                        }
                    }
                }
                return RMSNormIntermediates {
                    inputs: input.data.clone(),
                    rms_sq_vals: all_rms,
                    rsqrt_vals: all_rsqrt,
                    outputs: output_matrix.data.clone(),
            ...
            Err(e) => eprintln!("[rmsnorm] GPU failed, CPU fallback: {e}"),
        }
    }
    apply_rmsnorm_detailed(input, dim)
}
