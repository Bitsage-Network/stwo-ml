...
## BitSage Network Integration

BitSage uses STWO Circle STARKs for **True Proof of Computation** on GPU workloads:
...
cargo run --release --features gpu -- generate-proof \
  --job-id "job_12345" \
  --inputs "[1,2,3,4,5]" \
  --security-bits 64
```
...
BitSage's StwoVerifier contract (Sepolia) validates STWO proofs:
...
starkli invoke \
  ...
  submit_proof <proof_data>
```

### Proof Format (32 elements)

| Position | Field | Description |
|----------|-------|-------------|
| [0-3] | PCS Config | pow_bits, blowup, last_layer, n_queries |
| [4] | IO Commitment | `H(inputs \|\| outputs)` - binds proof to data |
| [5] | Trace Commitment | Merkle root of execution trace |
| [6-31] | FRI Data | 26 M31 field elements |

**Documentation**: [True Proof of Computation Guide](../../BitSage-Cairo-Smart-Contracts/docs/TRUE_PROOF_OF_COMPUTATION.md)
